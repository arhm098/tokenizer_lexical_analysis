...
int symbol_check(char *lexeme,char names[100][100])
{
	int n = 0;
	for(int i = 0;i<100;i++)
	{
...
int main()
{
	    	...
	    	if ((((buffer[R-1] >= 65) && (buffer[R-1] <= 122)) || ((buffer[R-1] >= 48) && (buffer[R-1] <= 57)) || buffer[R-1] == 46) &&(((buffer[R] < 65) || (buffer[R] > 122)) && ((buffer[R] <= 48) || (buffer[R] >= 57)) && buffer[R] != 46))
	    	{
	    		...
	    		if (res == -1) 
	    		{
	    			...
	    			if (isIdent == 0)
	    			{
						...
						int check = symbol_check(lexeme,names);
						printf("%d",check);
						if (check == -1)
						{
							for (int i = 0;i<strlen(lexeme);i++)
							{
								names[count_of_ident][i] = lexeme[i];
							}
							count_of_ident++;
						}
	...
	for (int i = 0;i<100;i++)
	{
		for(int j = 0;l<100;j++)
		{
			fprintf(symbolTable,"%c",names[i][j]);
		}
	}
	*/
    fclose(ptrSource);
